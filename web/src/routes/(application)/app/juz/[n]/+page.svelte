<script lang="ts">
  ...
  const prevHref = $derived(data.index > 1 ? juzPathFor(arabicCtx, data.index - 1) : null);
  const nextHref = $derived(
    data.index < RANGE_COUNTS[RangeKind.Juz] ? juzPathFor(arabicCtx, data.index + 1) : null,
  );
</script>
